...
def parse_args():
  ...
  parser.add_argument('--start_distance', type=float, default=-8.0,
                      help='Start point for manipulation in latent space. '
                           '(default: -3.0)')
  parser.add_argument('--end_distance', type=float, default=8.0,
    ...
    for interpolations_batch in model.get_batch_inputs(interpolations):
      for interpolation in interpolations_batch:
        if gan_type == 'pggan':
          ...
          outputs = model.easy_synthesize(np.array([interpolation]), **kwargs)
